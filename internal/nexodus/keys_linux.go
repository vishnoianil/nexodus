...
func CheckExistingKeys() (string, string, error) {
	publicKey, err := readKeyFile(LinuxPublicKeyFile)
	if err != nil {
		return "", "", err
	}
	privateKey, err := readKeyFile(LinuxPrivateKeyFile)
	if err != nil {
		return "", "", err
	}
...
func GenerateNewKeys() (string, string, error) {
	publicKey, privateKey, err := generateKeyPair(LinuxPublicKeyFile, LinuxPrivateKeyFile)
	if err != nil {
